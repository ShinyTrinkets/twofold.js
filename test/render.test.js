...
test('simple random integer', async t => {
  ...
  const tmp1 = await xfold.renderText(txt1)
  const tmp2 = await xfold.renderText(txt2)
  t.not(tmp1, txt1)
  t.not(tmp2, txt2)
...
test('render once', async t => {
  ...
  let tmp1 = await xfold.renderText(text)
  t.not(text, tmp1)
  let tmp2 = await xfold.renderText(tmp1)
  t.not(tmp1, tmp2)

  // Test with once=true
  text = `random <randomInt once=true></randomInt> ...`
  tmp1 = await xfold.renderText(text)
  t.not(text, tmp1)
  tmp2 = await xfold.renderText(tmp1)
  t.is(tmp1, tmp2)
})
...
test('simple sort render', async t => {
  ...
  const tmp = await xfold.renderText(txt)
  t.not(tmp, txt)
  li.sort()
...
test('emoji clock render', async t => {
  ...
  let tmp = await xfold.renderText(txt, { date: new Date(2012, 11, 21, 11, 11) })
  t.is(tmp.indexOf('clock'), 0)

  t.not(tmp, txt)
  t.true(tmp.indexOf('🕚') > 0)

  tmp = await xfold.renderText(txt, { date: new Date(2012, 11, 21, 11, 15), showHalf: false })
  t.true(tmp.indexOf('🕚') > 0)

  tmp = await xfold.renderText(txt, { date: new Date(2012, 11, 21, 12, 46), showHalf: false })
  t.true(tmp.indexOf('🕛') > 0)
})
...
test('separated sort render', async t => {
  ...
  let blob = li1.join('\n') + '\n\n' + li2.join('\n') + '\n\n' + li3.join('\n')
  let txt = `... <sort>\n${blob}</sort> ...`
  let tmp = await xfold.renderText(txt, {}, { sort: helpers.sortLines })
  t.not(tmp, txt)
  ...
  blob += '\n\n'
  txt = `??? <sort>\n${blob}</sort> ???`
  tmp = await xfold.renderText(txt, {}, { sort: helpers.sortLines })
  t.not(tmp, txt)
  t.is(tmp.length, txt.length)
  t.is(tmp.indexOf('???'), 0)
...
test('custom single tag', async t => {
  let tmp
  const mumu = () => 'ok'
  tmp = await xfold.renderText('<mumu />', {}, { mumu })
  t.is(tmp, 'ok')
  // Test open and close tag for single
  tmp = await xfold.renderText('<mumu />', {}, { mumu }, { openTag: '{', closeTag: '}' })
  t.is(tmp, '<mumu />')
  tmp = await xfold.renderText('{mumu /}', {}, { mumu }, { openTag: '{', closeTag: '}' })
  t.is(tmp, 'ok')
  // Test last stopper for single
  tmp = await xfold.renderText('<mumu />', {}, { mumu }, { lastStopper: '?' })
  t.is(tmp, '<mumu />')
  tmp = await xfold.renderText('<mumu ?>', {}, { mumu }, { lastStopper: '?' })
  t.is(tmp, 'ok')
  // Full test
  const cfg = { openTag: '{', closeTag: '}', lastStopper: '?' }
  tmp = await xfold.renderText('<mumu />', {}, { mumu }, cfg)
  t.is(tmp, '<mumu />')
  tmp = await xfold.renderText('{mumu ?}', {}, { mumu }, cfg)
  t.is(tmp, 'ok')
})
...
test('custom double tag', async t => {
  let tmp, cfg
  const mumu = () => 'ok'
  tmp = await xfold.renderText('<mumu></mumu>', {}, { mumu })
  t.is(tmp, '<mumu>ok</mumu>')
  // Test open and close tag
  cfg = { openTag: '{', closeTag: '}' }
  tmp = await xfold.renderText('<mumu></mumu>', {}, { mumu }, cfg)
  t.is(tmp, '<mumu></mumu>')
  tmp = await xfold.renderText('{mumu}{/mumu}', {}, { mumu }, cfg)
  t.is(tmp, '{mumu}ok{/mumu}')
...
test('deep custom function render', async t => {
  ...
  tmp = await xfold.renderText('<mumu><mumu></mumu></mumu>', {}, { mumu })
  t.is(tmp, '<mumu>ok</mumu>')
  t.is(calls, 2) // evaluate calls

  calls = 0
  tmp = await xfold.renderText('<mumu><mumu><mumu></mumu></mumu></mumu>', {}, { mumu })
  t.is(tmp, '<mumu>ok</mumu>')
  t.is(calls, 3) // evaluate calls

  calls = 0
  tmp = await xfold.renderText('<mumu><mumu /></mumu>', {}, { mumu })
  t.is(tmp, '<mumu>ok</mumu>')
  t.is(calls, 2) // evaluate calls
})

test('deep unknown function render', async t => {
  const tmp = await xfold.renderText('<mumu><mumu><mumu>\n<increment consume=true>0</increment></mumu></mumu></mumu>')
